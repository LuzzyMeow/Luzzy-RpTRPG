...
## 二十一、第十五轮改动（2026-06-19：UI/UX 完全重构 · 液态玻璃设计系统 · 全功能交叉实现）

### 21.0 改动背景

使用 `luzzy-skill-design` 技能与 `doc/` 目录参考组件（@lobehub/ui、rikkahub-master、AlibabaSans），完全重新设计 UI/UX。所有功能严格交叉对比原项目（assets/js/app.js + index.html），确保完全实现而非框架占位。采用液态玻璃（Glassmorphism）设计语言贯穿全局。

**硬约束**：保留所有内置 NSFW 提示词内容完整不修改；所有弹窗/容器/按钮/页面符合技能或组件包规范，不调用原生安卓样式。

### 21.1 液态玻璃设计系统（global.css 完全重写）
...
| 变量类别 | 变量名 | 亮色值 | 暗色值 |
|----------|--------|--------|--------|
| 透明度 | `--luzzy-glass-opacity` | 0.65 | 0.55 |
| 透明度（强） | `--luzzy-glass-opacity-strong` | 0.85 | 0.75 |
| 透明度（轻） | `--luzzy-glass-opacity-subtle` | 0.45 | 0.35 |
| 模糊半径 | `--luzzy-glass-blur` | 20px | 20px |
| 模糊半径（强） | `--luzzy-glass-blur-strong` | 30px | 30px |
| 模糊半径（轻） | `--luzzy-glass-blur-subtle` | 10px | 10px |
| 边框颜色 | `--luzzy-glass-border-color` | rgba(255,255,255,0.5) | rgba(255,255,255,0.12) |
| 高光渐变 | `--luzzy-glass-highlight-gradient` | 线性渐变 | 线性渐变 |
| 阴影 | `--luzzy-glass-shadow` | 0 4px 24px rgba(0,0,0,0.06) | 0 4px 24px rgba(0,0,0,0.3) |
| 内阴影 | `--luzzy-glass-inset-shadow` | inset 0 1px 1px rgba(255,255,255,0.5) | inset 0 1px 1px rgba(255,255,255,0.1) |
| 背景色 | `--luzzy-glass-bg` | rgba(255,255,255,0.65) | rgba(30,35,36,0.55) |

#### 21.1.2 玻璃工具类

| 类名 | 用途 | 强度 |
|------|------|------|
| `.luzzy-glass` | 标准玻璃容器 | blur(20px) saturate(180%) |
| `.luzzy-glass-strong` | 高强度玻璃容器 | blur(30px) saturate(200%) |
| `.luzzy-glass-subtle` | 轻量玻璃容器 | blur(10px) saturate(150%) |
| `.luzzy-glass-highlight` | 玻璃高光叠加层 | ::before 伪元素 |
| `.luzzy-card` | 玻璃卡片 | 标准+padding |
| `.luzzy-glass-bar` | 玻璃顶栏 | 高强度 |
| `.luzzy-glass-tabbar` | 玻璃底栏 | 高强度 |
| `.luzzy-glass-modal` | 玻璃弹窗 | 高强度+圆角 |
| `.luzzy-glass-input` | 玻璃输入框 | 轻量+内阴影 |
| `.luzzy-glass-button` | 玻璃按钮 | 轻量+交互态 |

#### 21.1.3 antd 组件玻璃化覆写

对 antd 的 Modal、Drawer、Collapse、Message 组件内容区应用玻璃背景，确保弹窗也呈现液态玻璃效果。

#### 21.1.4 动画

新增 `@keyframes luzzy-glass-fade-in`（淡入+缩放）和 `@keyframes luzzy-spin`（旋转加载）。

### 21.2 布局组件重构

#### 21.2.1 BottomTabBar.tsx（5-Tab 底部导航）

- **TRPG 独立菜单项**：5 个 Tab（聊天 / 角色 / **TRPG** / 工具 / 我的），TRPG 使用紫色强调色 `#8b5cf6`
- **玻璃底栏**：`background: var(--luzzy-glass-bg-strong)`, `backdrop-filter: blur(30px) saturate(200%)`
- **交互反馈**：Tab 激活时 `scale(1.1)`，按下时 `scale(0.92)`
- **TRPG 图标**：自定义游戏机形状 SVG
- **非激活 TRPG 提示**：紫色圆点 badge

#### 21.2.2 AppHeader.tsx（玻璃顶栏）

- **玻璃顶栏**：高强度玻璃背景
- **品牌渐变文字**：`linear-gradient(135deg, var(--luzzy-primary), var(--luzzy-tertiary))` + `-webkit-background-clip: text`
- **页面标题映射**：`/trpg` → 'TRPG', `/mine` → '我的'
- **嵌套路由回退**：先精确匹配，再首段路径回退，最后 'LUZZY'

#### 21.2.3 App.tsx（路由更新）

- 移除 Settings/More 路由，新增 `/trpg`（TrpgPage）和 `/mine`（MinePage）路由
- 懒加载 TrpgPage 和 MinePage
- ErrorBoundary 添加 `componentDidCatch` 错误日志
...
| 区域 | 玻璃强度 | 实现 |
|------|----------|------|
| 角色选择栏（顶栏） | 高强度 | `var(--luzzy-glass-bg-strong)` + `blur(30px)` |
| 消息气泡（助手） | 标准 | `var(--luzzy-glass-bg)` + `blur(20px)` |
| 消息气泡（用户） | 主色 | `var(--luzzy-primary)` 实色 |
| 输入区（底栏） | 高强度 | `var(--luzzy-glass-bg-strong)` + `blur(30px)` |
| 角色选择弹窗项 | 轻量 | `var(--luzzy-glass-bg-subtle)` + `blur(10px)` |
| 角色描述抽屉 | 高强度 | `var(--luzzy-glass-bg-strong)` + `blur(30px)` |
| 思考链卡片 | 轻量 | antd Collapse + 玻璃覆写 |
| 工具调用卡片 | 轻量 | 左边框状态色 + 玻璃背景 |
| 记忆召回卡片 | 轻量 | 左边框 tertiary + 玻璃背景 |

#### 21.3.2 新增功能（交叉对比原项目实现）

| 功能 | 原项目实现 | 新实现 |
|------|-----------|--------|
| 角色背景层 | `useCharacterBackground` + blur(2px) | blur(8px) saturate(1.2) opacity(0.25) |
| 上下文长度指示器 | `totalContextLength` + `chatRoundStats.floors` | `estimateTokens` 估算 + `chatFloors` 楼层数 |
| 角色描述面板 | 下拉面板 `showDescriptionPanel` | Drawer 抽屉（角色简介/性格/场景/开场白/标签/创作者） |
| 思考链展示 | `native-thinking-card` 时间线 | antd Collapse 玻璃卡片 |
| 工具调用展示 | 时间线步骤 + 状态图标 | 玻璃卡片 + 状态标签 + 查询/原因/结果折叠 |
| 记忆召回展示 | `memory-recall-detail` 分片列表 | 玻璃卡片 + Tag 标签（轮次/相似度） |
| 生成耗时 | 无显式展示 | `generationTime` 转秒显示 |
| 消息头像 | 角色头像 + 用户首字母 | 角色头像 + 用户渐变背景 |
| 名称标签 | 气泡外名称 | 玻璃名称标签 |
...
const TOOL_STATUS_LABEL = {
  pending: '等待中', receiving: '接收中', queued: '排队中',
  running: '执行中', continuing: '续写中',
  completed: '已完成', error: '失败'
};
```

状态颜色：completed（tertiary 绿）、error（error 红）、running（primary 青）。

### 21.4 TrpgPage 新建（TRPG 独立模式）

- **iframe 嵌入**：`https://aisandboxgame.com/`，sandbox 允许同源/脚本/表单/弹窗
- **首次说明弹窗**：Modal 配置步骤说明，`sessionStorage` 记忆"本次不再提示"
- **代理配置弹窗**：显示 API 地址/Key/模型名/深度思考状态，"重新推送配置"按钮写入 `localStorage`
- **玻璃加载遮罩**：spinner + "正在加载 TRPG 模式"
- **工具栏**：刷新按钮 + 代理配置按钮（玻璃圆形按钮）
- **配置推送优化**：`useCallback` 依赖改为实际使用的 5 个字段，避免不必要重建

### 21.5 CharactersPage 重构

#### 21.5.1 PNG 角色卡导入（交叉对比原项目）

- **tEXt chunk 解析**：keyword "chara"/"ccv3" 的 base64 编码 JSON
- **UTF-8 安全 Base64**：`utf8ToBase64` / `base64ToUtf8` 编解码
- **CRC32 校验**：PNG chunk 完整性验证

#### 21.5.2 头像上传

- **图片压缩**：`compressImage`（300px 宽度，JPEG 质量 0.7）
- **编辑器内预览**：圆形预览 + 上传/移除按钮
...
| 格式 | 实现 |
|------|------|
| PNG | CRC32 + tEXt chunk 注入 |
| JSON | 完整角色卡 JSON |
| 聊天记录 | 导出当前角色对话历史 |

#### 21.5.4 玻璃拟态角色卡片

角色列表卡片应用 `luzzy-glass` 样式，收藏/编辑/删除按钮玻璃化。
...
| 方式 | 实现 |
|------|------|
| GitHub 导入 | 3 种 URL 格式 + 4 个镜像站（gh-proxy.com/github.moeyy.xyz/ghfast.top），GitHub API 优先 + raw.githubusercontent.com 回退 |
| ZIP 导入 | JSZip 解析，不区分大小写查找 SKILL.md，优先路径最浅 |
| 手动粘贴 | 直接输入 SKILL.md 内容 |
...
- 新建文件夹/文件
- 编辑文件内容
- 多文件存储在 IndexedDB `skillFiles_<toolId>` 键下

#### 21.6.3 向量搜索 UI

- 搜索框 + TopK 选择器
- 相似度分数展示
- 三 Tab 布局（内置工具 / MCP 工具 / SKILL）

### 21.7 MinePage 新建（合并设置+更多）

合并原 SettingsPage + MorePage 为单页，10 个 Collapse 分区：

1. **API 连接与服务**：API 地址/Key/模型名/流式/深度思考/自定义请求体
2. **生成参数**：温度/TopP/TopK/重复惩罚/最大长度
3. **记忆引擎**：启用/嵌入模型/最大记忆/召回深度/TopK/相似度阈值/压缩
4. **预设管理**：内置预设 + 自定义预设 CRUD
5. **世界书**：条目 CRUD + 关键词/扫描深度
6. **正则脚本**：脚本 CRUD + 查找/替换/作用域
7. **UI 模板**：模板 CRUD
8. **用户档案**：名称/描述/人称
9. **外观**：主题模式切换
10. **关于 LUZZY**：版本信息

**关键决策**：供应商编辑使用 `useSettingsStore.setState` 直接非破坏性更新；全局记忆存储键使用 `global_memory`（修复旧 MorePage 的 bug）；NSFW 预设内容完整保留。

### 21.8 服务层接入

#### 21.8.1 toolService → useChatStore（工具调用循环）

- 导入 `findPendingActiveToolCallInText`, `executeActiveToolCall`, `filterToolsForCharacter`
- 工具调用循环（最多 5 次迭代）：扫描 → 执行 → 更新 toolCalls → 添加工具结果用户消息 → 续写
- **abort 信号检查**：每次迭代开头检查 `abortController.signal.aborted`
- **catch 块数据一致性**：重新获取最新消息追加 error 记录

#### 21.8.2 memoryService → chatService（向量记忆系统）

- `buildContext` 改为 `async`，返回 `Promise<BuildContextResult>`
- 新增向量记忆召回段（注入 `<memory_recall>` 块到系统提示词）
- `extractMemory` 用 `buildVectorMemory` 替换 stub，包含轮次号计算、分片合并、`maxMemories` 截断、IndexedDB 持久化
- 移除残留调试 `console.log`

### 21.9 类型系统扩展（types/index.ts）

新增类型：

```typescript
export interface Conversation { id, title, characterId?, messages, createdAt, updatedAt, pinned? }
export interface TrpgProxyConfig { apiUrl, apiKey, modelName, enableThinking, customRequestBody }
export interface SkillFileNode { name, path, isDirectory, content?, children? }
export type SkillImportSource = 'github' | 'zip' | 'manual'
export interface GithubMirror { name, url }
export type CharacterExportFormat = 'png' | 'json' | 'chat'
export interface ContextViewerItem { role, content, source, tokenEstimate? }
export interface ModelInfo { id, name, providerId, providerName, capabilities?, contextLength?, favorite? }
...
| # | 文件 | 问题 | 修复 |
|---|------|------|------|
| 1 | useChatStore.ts | DEFAULT_MEMORY_SETTINGS 缺 embeddingApiProviderId | 补充字段 |
| 2 | useChatStore.ts | 工具调用循环未检查 abort 信号 | 添加 `if (abortController?.signal.aborted) break` |
| 3 | useChatStore.ts | catch 块使用过时 toolCalls | 重新获取最新消息追加 error |
| 4 | CharactersPage.tsx | extractPngCardData 返回 any | 改为 `Record<string, unknown> \| null` |
| 5 | CharactersPage.tsx | loadChatHistory 浮动 Promise 无错误处理 | 添加 reject 回调 |
| 6 | chatService.ts | 残留调试 console.log | 移除 |
| 7 | types/index.ts | TabItem.icon 类型过于宽泛 | 新增 TabIconName 类型 |
| 8 | BottomTabBar.tsx | TabIcon name 参数类型不安全 | 使用 TabIconName |
| 9 | App.tsx | ErrorBoundary 未记录错误 | 添加 componentDidCatch |
| 10 | AppHeader.tsx | PAGE_TITLES 无法匹配嵌套路由 | 增加首段路径回退 |
| 11 | TrpgPage.tsx | pushApiConfigToNative 依赖整个 settings | 改为 5 个实际字段 |

### 21.11 文件改动清单

| 文件 | 改动类型 | 说明 |
|------|----------|------|
| `frontend/src/styles/global.css` | 重写 | 液态玻璃设计系统（变量+工具类+antd覆写+动画） |
| `frontend/src/components/layout/BottomTabBar.tsx` | 重写 | 5-Tab 玻璃底栏，TRPG 独立菜单 |
| `frontend/src/components/layout/AppHeader.tsx` | 重写 | 玻璃顶栏，渐变品牌文字 |
| `frontend/src/App.tsx` | 修改 | 路由更新，ErrorBoundary 错误日志 |
| `frontend/src/pages/ChatPage.tsx` | 重写 | 液态玻璃+工具调用+记忆召回+角色描述抽屉+上下文指示器 |
| `frontend/src/pages/TrpgPage.tsx` | 新建 | TRPG 独立模式 iframe + 代理配置 |
| `frontend/src/pages/MinePage.tsx` | 新建 | 合并设置+更多，10 分区单页 |
| `frontend/src/pages/CharactersPage.tsx` | 修改 | PNG 导入+头像上传+三格式导出 |
| `frontend/src/pages/ToolsPage.tsx` | 修改 | SKILL GitHub/ZIP 导入+文件管理器+向量搜索 |
| `frontend/src/store/useChatStore.ts` | 修改 | 工具调用循环+向量记忆+abort 检查 |
| `frontend/src/services/chatService.ts` | 修改 | buildContext 异步化+向量召回+extractMemory 实现 |
| `frontend/src/types/index.ts` | 修改 | 新增 10+ 类型定义 |
...
- [x] TypeScript 类型检查通过（`tsc -b --noEmit` exit code 0）
- [x] Vite 构建成功（25.14s，ChatPage 34.21kB）
- [x] 静态代码审查完成（10 处修复）
- [x] NSFW 预设内容完整保留
- [x] TRPG 模式作为独立菜单项存在
- [x] 所有页面应用液态玻璃设计
- [x] 所有弹窗/容器/按钮使用玻璃样式或 antd 组件
- [x] 工具调用循环正确接入（最多 5 次迭代）
- [x] 向量记忆系统正确接入（buildContext + extractMemory）
- [x] 角色描述抽屉展示完整角色信息
