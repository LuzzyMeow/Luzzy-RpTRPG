...
import type { SkillName, ActionCategory, TrpgCharacter, TrpgGameState, WorldCard, InventoryItem, GameNpc, GameLocation } from '~/types/trpg';
  ...
  inventory_add: {
    ...
    parameters: {
      ...
      properties: {
        ...
        item_type: { type: 'string', description: '物品类型（weapon/armor/consumable/quest/misc）' },
 ...
 * @param executor 执行函数
 */
export function registerTrpgToolExecutor(
  toolName: string,
  executor: TrpgToolExecutor,
): void {
...
registerTrpgToolExecutor('eval_world_rules', (args, ctx) => {
  ...
  const laws = ctx.worldCard?.laws ?? [];
  const mods = ctx.worldCard?.mods ?? [];
  const triggeredLaws: string[] = [];
  const triggeredMods: string[] = [];

  for (const law of laws) {
    if (law.triggerCondition && category.includes(law.triggerCondition.toLowerCase())) {
      triggeredLaws.push(`${law.name}: ${law.effect}`);
    }
  }

  for (const mod of mods) {
    if (mod.effect && category.includes(mod.effect.toLowerCase().split(' ')[0])) {
      triggeredMods.push(`${mod.name}: ${mod.effect}`);
    }
  }
